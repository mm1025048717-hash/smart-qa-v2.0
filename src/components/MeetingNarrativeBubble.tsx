...
const NARRATIVE_SECTIONS: { title: string; items: NarrativeItem[] }[] = [
  {
    ...
    items: [
      ...
      { text: '四个设计哲学：千人千面（CEO 和开发看到的引导完全不同）、按需加载（点开数字员工才讲数字员工）、有的放矢（引导是为了教会完成任务）、无缝流转（引导结束变成右下角小助手）。', link: { label: '前往小助手', target: 'helper-panel' } },
      { text: '四类角色：CXO / 业务负责人 / 一线业务 / 数据开发。欢迎页必须选角色，决定后续菜单复杂度和引导路径。', link: { label: '前往角色选择', target: 'role-picker' } },
      { text: '用户旅程：全屏欢迎蒙版 → 角色选择（四张卡片必选）→ 分角色 Spotlight 高光引导（聚焦、不可点击蒙版关闭、有跳过二次确认）→ 最后一步「完成」时蒙版缩到右下角变成亿问小助手。', link: { label: '前往小助手', target: 'helper-panel' } },
      ...
      { text: '一线业务：聚焦指标口径、知识库、新建术语；右下角悬浮球求助与上报。', link: { label: '能力与数字员工', target: 'employee-cards' } },
